...
function solve() {
	var library = (function () {
		...
		function addBook(book) {
			...
			if (parameterAlreadyExist(book.title, 'title')) {
				throwBookNotUniqueError('title');
			}

			if (parameterAlreadyExist(book.isbn, 'isbn')) {
		...
		function parameterAlreadyExist(name, type) {
			for (var ind= 0, len = books.length; ind < len; ind += 1) {
		...
		function checkAllBookParameters(book, lenghtOfParams) {
			if (Object.keys(book).length !== lenghtOfParams) {
